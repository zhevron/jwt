...
-----END PUBLIC KEY-----`)
var PrivateKeyPKCS1 = []byte(`-----BEGIN EC PRIVATE KEY-----
...
func TestVerifyES256_Fail(t *testing.T) {
	if VerifyES256(Token, SignatureES256, []byte("INVALID")) == nil {
		t.Fatal("expected non-nil, got nil")
	}
}

func TestVerifyES384_Fail(t *testing.T) {
	if VerifyES384(Token, SignatureES384, []byte("INVALID")) == nil {
		t.Fatal("expected non-nil, got nil")
	}
}

func TestVerifyES512_Fail(t *testing.T) {
	if VerifyES512(Token, SignatureES512, []byte("INVALID")) == nil {
		t.Fatal("expected non-nil, got nil")
	}
}
